...
class Search:
    ...
    def IDA_star(self, initial_state):
        ...
        while True:
            self.visited.clear()  
            found, new_path, expanded_nodes, memory_consumed, new_threshold = self.search(initial_state, 0, threshold, [])
    ...
    def search(self, current_state, g, threshold, path):
        ...
        for neighbor in self.get_neighbors(current_state):
            if tuple(neighbor) not in self.visited:
                ...
                found, new_path, expanded, mem_used, temp_threshold = self.search(neighbor, g + 1, threshold, path + [move])
    ...
    def solve(self, input):
        ...
        path, expanded_nodes, time_taken, memory_consumed = self.IDA_star(initial_list)
        if path:
